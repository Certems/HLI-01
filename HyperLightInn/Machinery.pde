class Pump{
  ...
  PVector pos;                    //Position of tile it is on, relative coords
  ...
  Pump(PVector position, int tileNumber){
    pos = position;
    tileNum = tileNumber;
  }
}
...
class Vat{
  ...
  PVector pos;                    //Position of tile it is on, relative coords
                                  ...
                                  //**Index these values are at indicate their fluid type**
  float fluidIn;       //##JUST HOLDS ONE FOR NOW##If multiple different fluids inside machine, input /sec 
  float fluidOut;      //##JUST HOLDS ONE FOR NOW##If multiple different fluids inside machine, output /sec
  float fluidWithin;   //##JUST HOLDS ONE FOR NOW##If multiple different fluids inside machine, current volume within
  
  Vat(PVector position, int tileNumber){
    pos = position;
    tileNum = tileNumber;
  }
}
...
class Machinery{
  ...
  Machinery(PVector position, int tileNumber){
    pos = position;
    tileNum = tileNumber;
  }
}
...
void addVatToList(PVector pos, int n){
  ...
  {
    Tiles.get( n ).inputterInd = vatList.size();  //##WILL BREAK IF A VAT IS REMOVED##//
...
void addMachineryToList(PVector pos, int n){
  ...
  {
    Tiles.get( n ).inputterInd = machineryList.size();
