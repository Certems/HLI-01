...
ArrayList<Integer> oneTilePath(int pathStart, int pathEnd){     //For 1x1 entities
    ...
    while ( pathFinal.size() == 0 )  //While end not found
    {
        ...
        for(int j=-1; j<=1; j+=2)   //Checks (j= -1) and (j= 1) --> Tiles below and above
        {
            ...
            if( ( Tiles.get(pathCheckTile).pathChecked == false ) && ( collideables.get( Tiles.get(pathCheckTile).index ) == 0 ) )
            {
        ...
        for(int j=-1; j<=1; j+=2)   //Checks (j= -1) and (j= 1) --> Tiles below and above
        {
            ...
            for(int q=-1; q<2; q++)
            {
                for(int p=-1; p<2; p++)
                {
                    ...
                    if( collideables.get( Tiles.get(pathCheckTileTemp).index ) == 1 )    //If tile is collideable...
                    {
            ...
            //--- Collideable and pathChecked check
            if( (( Tiles.get(pathCheckTile).pathChecked == false ) && ( collideables.get( Tiles.get(pathCheckTile).index ) == 0 )) && (pathBlocked == false) )
            {
