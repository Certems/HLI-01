void findHoveredIndex(int n){
  if(Tiles.get(n).index == 8)   //If interacting with tree
    {
     addItemToInventory("Sticks", 1, sticks);
    }
  if(Tiles.get(n).index == 9)   //If interacting with water
  {
    addItemToInventory("Water bucket", 1, waterBucket);
  }
}


void addItemToInventory(String itemName, int quantity, PImage itemIconTemp){
  ...
  {
    user.inventoryIcons.add(itemIconTemp);
...
void addItemToContainer(String itemName, int quantity, PImage itemIcon, int containerType, int containerNum){
  ...
  {
    containerList.get(containerType).get(containerNum).contentIcon    .add(itemIcon);
...
void drawItemIcon(String itemName, PVector centrePos){
  ...
  for(int i=0; i<itemCatalogueNames.size(); i++)
  {
    ...
    if(itemName == "Sticks")
    {
      image(sticks, centrePos.x, centrePos.y);
    }
    if(itemName == "Water bucket")
    {
      image(waterBucket, centrePos.x, centrePos.y);
    }
    if(itemName == "Spade")
    {
      image(spade, centrePos.x, centrePos.y);
    }
    if(itemName == "Seeds")
    {
      image(seeds, centrePos.x, centrePos.y);
    }
    if(itemName == "Fertiliser")
    {
      image(fertiliser, centrePos.x, centrePos.y);
    }
    if(itemName == "Manual")
    {
      image(userManual, centrePos.x, centrePos.y);
    }
  }
  popStyle();
