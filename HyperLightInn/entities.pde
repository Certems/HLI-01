...
class entity{
    ...
    ArrayList<Integer> givenIndexList      = new ArrayList<Integer>();  //List of all tiles of a given index on the board, used in path finding
    ArrayList<Integer> availableIndexList  = new ArrayList<Integer>();  //List of all tiles of a given index on the board that can be reached in a path
    ...
    PVector calcPathEnds(int requiredIndex){
        //calc start point (uses orc's stored position)
        pathEnds.x = convertPosToTile( pos );

        //(0)Check see what type of index is being looked for (e.g a seat, a container, etc)
        findingSeat    = false;
        findingCounter = false;
        //## CAN HAVE ANOTHER BREAK CONDITION TO STOP SEARCHING OTHER SETS ONCE YOURS IS FOUND ##
        for(int i=0; i<containerCatalogue.size(); i++)
        {
            //If a condition related to this needs to be found, insert here...
        }
        for(int i=0; i<counterCatalogue.size(); i++)
        {
            if(counterCatalogue.get(i) == requiredIndex){
                findingCounter = true;
                break;
            }
        }
        for(int i=0; i<seatCatalogue.size(); i++)
        {
            if(seatCatalogue.get(i) == requiredIndex){
                findingSeat = true;
                break;
            }
        }
        ...
        givenIndexList.clear();
        availableIndexList.clear();
        for(int i=0; i<Tiles.size(); i++)           //Go through al tiles...
        {
        if( Tiles.get(i).index == requiredIndex ) //If index of given tile is what we are looking for...
        {
            givenIndexList.add(i);                  //Then add the tile number to givenIndexList
        }
        }

        //(2)Check if list is empty, if so return a null value
        if(givenIndexList.size() == 0)  //If none of given index found...
        {
        ...
        for(int i=0; i<givenIndexList.size(); i++)     //Check them all to find shortest distance  //## EDIT NEEDED HERE TO ENSURE IT IS ALSO REACHABLE ##//
        {
            pathResult = tilePathing(int(pathEnds.x), int( givenIndexList.get(i) ), entitySize);
            if( pathResult.get(0) != -1 ){
                availableIndexList.add( givenIndexList.get(i) );
            }
        }
        ...
        {
            //Find first base value
            if(findingSeat == true)
            {
                for(int j=0; j<availableIndexList.size(); j++)
                {
                    if( Tiles.get( availableIndexList.get(j) ).tileUsed == false )
                    {
                        tileTemp             = availableIndexList.get(j);              //Make first tile (that is available) shortest by default
            ...
            //##Add else ifs here if needed (e.g more conditions added)##
            else
            {
                tileTemp             = availableIndexList.get(0);              //Make first item shortest by default
                pathTileDiffX        = abs(        (tileTemp % colNum)   -        (pathEnds.x % colNum)   );
                pathTileDiffY        = abs( floor( (tileTemp / colNum) ) - floor( (pathEnds.x / colNum) ) );
                leadingDist          = sqrt( pow(pathTileDiffX ,2) + pow(pathTileDiffY ,2) );
                destinationAvailable = true;
            }
            if(destinationAvailable == true)
            {
                for(int i=0; i<availableIndexList.size(); i++)
                {
                    //Calc dists here
                    pathTileDiffX        = abs(        (availableIndexList.get(i) % colNum)   -        (pathEnds.x % colNum)   );
                    pathTileDiffY        = abs( floor( (availableIndexList.get(i) / colNum) ) - floor( (pathEnds.x / colNum) ) );
                    workingDist          = sqrt( pow(pathTileDiffX ,2) + pow(pathTileDiffY ,2) );
                    if( workingDist < leadingDist )               //If this item is a shorter distance...
                    {
                        if( (findingSeat == true) || (findingCounter == true) )
                        {
                            if( Tiles.get( availableIndexList.get(i) ).tileUsed == false ){
                                tileTemp    = givenIndexList.get(i);          //Then make it the leading value
                                leadingDist = workingDist;
                            }
                        }
                        //## ADD ELSE IFS IF MORE CONDITIONS, E.G FOR CONTAINERS, ARE ADDED ##
                        else
                        {
                            tileTemp    = givenIndexList.get(i);          //Then make it the leading value
                            leadingDist = workingDist;
                ...
                //Once the final index tile has been found...
                if( (findingSeat == true) || (findingCounter == true) )    //If is a seat
                {
                    Tiles.get( tileTemp ).tileUsed = true;
                }
            }
            else    //## MAY NOT BE NEEDED, MAY CAUSE PROBLEMS ##
            {
        ...
        //println("availableIndexList; ",availableIndexList);
        //println("--");
  ...
  void calcBehaviour(){
    ...
    if( (currentDrink.get(0) != -1) && (paidDrink == false) ){
        ...
        Tiles.get( convertPosToTile(pos) ).tileUsed = false;
        calcPathing(5);
        hasDrink  = true;
        paidDrink = true;
