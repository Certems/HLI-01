...
void GeneralisedBugFixing(){
  ...
  print  ("MousedTile Index; ",Tiles.get( findMouseTile() ).index);print("   ///   ");println("MousedTile Floor; ",Tiles.get( findMouseTile() ).floorInd);
  print  ("tileIndSel      ; ",tileIndSel);                        print("   ///   ");println("floorIndSel; ",floorIndSel);
  println("---");
  ...
  if( (Tiles.get( findMouseTile() ).index == 11) || (Tiles.get( findMouseTile() ).index == 12) )  //If a vat or machinery...
  {
    if(Tiles.get( findMouseTile() ).inputterInd > -1)  //##FAILSAFE##//
    {
      if(Tiles.get( findMouseTile() ).index == 12){
      println("VatFluidIn; ", vatList.get( Tiles.get( findMouseTile() ).inputterInd ).fluidIn );}
      if(Tiles.get( findMouseTile() ).index == 13){
      println("MachineryFluidIn; ", machineryList.get( Tiles.get( findMouseTile() ).inputterInd ).fluidIn );}
    }
  }
...
void displayInventory(){
  ...
  for(int i=0; i<invDisplayNum; i++)
  {
    ...
    imageMode(CENTER);
    image(user.inventoryIcons.get(i),   bubbleBorder + ( (i+1)*(screenWidth - 2*bubbleBorder) / ( invDisplayNum+1 ) ),  (bubbleLen.y));         //Texture
...
void displayTileIndSel(){
  pushStyle();
  textSize(15);
  textAlign(LEFT);
  fill(0);
  text(tileIndSel, mouseX-10, mouseY-10);
  popStyle();
}
void displayFloorIndSel(){
  pushStyle();
  textSize(13);
  textAlign(LEFT);
  fill(0);
  text(floorIndSel, mouseX+10, mouseY+10);
  popStyle();
}
...
void modeSpecificOverlays(){
  ...
  if(modeSelector==1)  //Building mode
  {
    ...
    //-------------------------//
    if(tileIndSel == 7)  //undefined multi-tile
    {
      displayGhost(findMouseTile(), 3, 3);  //##SEPARATE CALCULATIONS AND DRAWING##//
    }
    if(tileIndSel == 10)  //undefined multi-tile
    {
      displayGhost(findMouseTile(), 3, 3);  //##SEPARATE CALCULATIONS AND DRAWING##//
    }
    if(tileIndSel == 17)  //undefined multi-tile
    {
      displayGhost(findMouseTile(), 7, 7);  //##SEPARATE CALCULATIONS AND DRAWING##//
    }
    if(tileIndSel == 18)  //undefined multi-tile
    {
