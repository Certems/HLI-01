...
boolean findingSeat      = false;         //Used in path finding, whether or not the tile to path to is a seat, and if so to check if it is free or not
boolean findingCounter   = false;         //
boolean destinationAvailable     = false; //States whether, for path finding, an end tile can be found
boolean iconFound                = false; //Whether or not the icon for an item being moved is found
boolean tradingOutpostMenuToggle = false; //Whetehr or not to show the trading outpost shop screen
...
int frameTotal;
int perFrame;
int tileIndSel  = 0;           //The index the main tile will be changed to when given key pressed
int floorIndSel = 0;           //The index the floor tile will be changed to when given key pressed
int squarePos;
...
int catalogueType     =  0;         //Which type of items the catalogue will show (e.g tiles, floor, invItems, etc)
int catalogueTypeMax  =  2;     //**Will need updating if more types are added, but should update if custom types are added
int tradingOutpostInd = -1;         //The tileNum of the last tradingOutpost selected, in order to show its contents
...
float catalogueTypeSpaceX = -10;           //How much space the catalogue type changer has (in the x and y direction) when drawn next to the pie selector
float catalogueTypeSpaceY = -10;           //
float catalogueTypeWidth  = 40.0;          //
float pieSelectorRadius = 4*wallWidth;     //Pie selector radius
float pieSelectorPosX   = screenWidth/2;   //Pie selector centre X position
float pieSelectorPosY   = screenHeight/2;  //Pie selector centre Y position
...
ArrayList<ArrayList<Integer>> pieSelected     = new ArrayList<ArrayList<Integer>>();    //List of the categories on the pie wheel   ## MAY NEED TO EXPAND, FOR MULTIPLE WHEELS / WHEELS INTO WHEELS ##

ArrayList<String> tileCatalogueNames    = new ArrayList<String>();   //List of all items available to be selected in the large list; Their names at given index
ArrayList<PImage> tileCatalogueIcons    = new ArrayList<PImage>();   //List of all items available to be selected in the large list; Their icons at given index
ArrayList<String> floorCatalogueNames   = new ArrayList<String>();   //
ArrayList<PImage> floorCatalogueIcons   = new ArrayList<PImage>();   //
ArrayList<String> itemCatalogueNames    = new ArrayList<String>();   //
ArrayList<PImage> itemCatalogueIcons    = new ArrayList<PImage>();   //
ArrayList<String> currentCatalogueNames = new ArrayList<String>();   //
ArrayList<PImage> currentCatalogueIcons = new ArrayList<PImage>();   //

IntList drinkPrices        = new IntList(1,3,2);   //Prices of each type of beer per ml
IntList containerCatalogue = new IntList(2,17);    //The indices of containers available
IntList seatCatalogue      = new IntList(5,11);     //The indices of seats available       ##BROKEN WITH JUST 1 ITEM##
IntList counterCatalogue   = new IntList(6,11);     //The indices of seats available       ##BROKEN WITH JUST 1 ITEM##

IntList tileCataloguePrices    = new IntList( 2,5,20,50,25,100,125,0,0,0,250,300,350,20,0,75,0,0);
IntList tileCatalogueQuantity  = new IntList(50,5, 2, 4, 4,  5,  9,1,1,1,  2,  2,  2, 4,1, 4,1,1);
IntList floorCataloguePrices   = new IntList( 0, 5);
IntList floorCatalogueQuantity = new IntList(10,10);
IntList itemCataloguePrices    = new IntList( 0, 0,75,150,500,100);
IntList itemCatalogueQuantity  = new IntList(10,10, 5, 20, 10,  1);
...
//Temp values
PImage itemExchangeIcon;

//Custom menus / screens
PImage tradingOutpostMenu;
