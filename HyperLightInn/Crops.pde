...
void placeCrop(){
  ...
  if(Tiles.get( tileNumTemp ).index == 14)                                                    //Can only place on fields
  {
    if( int(Tiles.get( tileNumTemp ).cropInd.x) == -1 )                                       //If no crop already here...
    {
      if(cropSelector == 0)  //If wheat...
      {
        //cropList; 1st = what type of crop, 2nd = the crop itself
        //cropInd points to both
        Tiles.get( tileNumTemp ).cropInd.x  = cropSelector;                                                //Updates cropInd
        Tiles.get( tileNumTemp ).cropInd.y  = cropList.get( cropSelector ).size();                         //
        Wheat newWheat = new Wheat( Tiles.get( tileNumTemp ).pos, Tiles.get( tileNumTemp ).n);
      ...
      if(cropSelector == 1)  //If barley...
      {
        ...
        Tiles.get( tileNumTemp ).cropInd.x  = cropSelector;                                                //Updates cropInd
        Tiles.get( tileNumTemp ).cropInd.y  = cropList.get( cropSelector ).size();                         //
        Barley newBarley = new Barley( Tiles.get( tileNumTemp ).pos, Tiles.get( tileNumTemp ).n);
...
void removeCrop(int tileNumTemp){
  cropType = int(Tiles.get(tileNumTemp).cropInd.x);
  if( Tiles.get( tileNumTemp ).cropInd.y > -1 )
  {
    //Decrease index of all crops above one being removed
    for (int i=int(Tiles.get( tileNumTemp ).cropInd.y)+1; i<cropList.get( cropType ).size(); i++ )
    {
      Tiles.get( cropList.get(cropType).get(i).tileNum ).cropInd.y -= 1;  //Decrease list position
    }
    println("cropRemoved");                                      //*****//
    //println("x; ",int(Tiles.get( tileNumTemp ).cropInd.x));      //*****//
    //println("y; ",int(Tiles.get( tileNumTemp ).cropInd.y));      //*****//
    cropList.get( int(Tiles.get( tileNumTemp ).cropInd.x) ).remove( int(Tiles.get( tileNumTemp ).cropInd.y) );  //Remove crop  //##NOT WORKING, -1 IND SOMEWHERE??##//
    Tiles.get( tileNumTemp ).cropInd.x = -1;                                                                    //Change respective cropInd to have no crop
    Tiles.get( tileNumTemp ).cropInd.y = -1;                                                                    //
  }
...
void harvestCrop(int tileNumTemp){
  ...
  if(int(Tiles.get(tileNumTemp).cropInd.x) > -1)
  {
    //(1)Determine crop
    if( int(Tiles.get(tileNumTemp).cropInd.x) == 0 )
    {
      //(2)If crop at end of life
      Crop currentCrop = cropList.get( int(Tiles.get(tileNumTemp).cropInd.x) ).get( int(Tiles.get(tileNumTemp).cropInd.y) );
      if( ((Wheat)currentCrop).lifeCond3 == true  )  //**Replace with max life cond / life cond where crop is FIRST harvestable**//
      {
        ...
        addItemToInventory("Wheat", 5, barleyStg1);
        //(4)Remove crop
    ...
    if( int(Tiles.get(tileNumTemp).cropInd.x) == 1 )
    {
      ...
      {
        //(3)Add crop to inventory
        addItemToInventory("Barley", 5, barleyStg4);
        //(4)Remove crop
        removeCrop(tileNumTemp);
      }
    }
