    ...
    if(startPipeFound == true)                          //If there is an output pipe to cause a flow...
    {
      ...
      for(int j=0; j<borderIndicesFinal.size(); j++)
      {
        ...
        if( borderIndicesFinal.get(j) == 12 )  //If vat present
        {
          if(j==0){                   //j is ignoreDir, we need lastDir
            startPipe.lastDir=7;
            if( vatList.get( Tiles.get( startPipe.tileNum -colNum ).inputterInd ).fluidIn >0 ){  //If this inputter has fluid in it
            nearFluidSource = true;}
          }
          if(j==1){
            startPipe.lastDir=5;
            if( vatList.get( Tiles.get( startPipe.tileNum -1 ).inputterInd ).fluidIn >0 ){  //If this inputter has fluid in it
            nearFluidSource = true;}
          }
          if(j==2){
            startPipe.lastDir=3;
        ...
        if( borderIndicesFinal.get(j) == 13 )  //If machinery present
        {
          if(j==0){                   //j is ignoreDir, we need lastDir
            startPipe.lastDir=7;
            if( machineryList.get( Tiles.get( startPipe.tileNum -colNum ).inputterInd ).fluidIn >0 ){  //If this inputter has fluid in it
            nearFluidSource = true;}
          }
          if(j==1){
            startPipe.lastDir=5;
            if( machineryList.get( Tiles.get( startPipe.tileNum -1 ).inputterInd ).fluidIn >0 ){  //If this inputter has fluid in it
            nearFluidSource = true;}
          }
          if(j==2){
            startPipe.lastDir=3;
...
ArrayList findBorderTilesInd(int n){
  ...
  //Search within table
  //Up
  if( Tiles.get(n-colNum).index > -1 ){
  borderIndices.add( int(Tiles.get(n-colNum).index) );}
  else{
  borderIndices.add( -1 );}
  
  //Left
  if( Tiles.get(n-1).index > -1 ){
  borderIndices.add( int(Tiles.get(n-1).index) );}
  else{
  borderIndices.add( -1 );}
